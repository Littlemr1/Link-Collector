...
class Collector:
    def __init__(self):
        ...
        Button(self.root,text="Guardar URL como:",width=49,bg="gray77",command=self.enter_name).place(x=5,y=70)#79
        Button(self.root,text="Limpiar URL",width=28,bg="gray77",command=self.clear_url).place(x=363,y=70)
        ...
        Button(self.root,text="BUSCAR",bg="gray77",command=self.search_name).place(x=513,y=110)
        self.showAll = Button(self.root,text="MOSTRAR TODOS LOS LINKS",width=28,command=self.show_all)
        ...
        Button(self.root,text="IMPORTAR NUEVO LINK",bg="gray77",width=28,height=2,command=self.init_copy).place(x=363,y=230)#245
        Button(self.root,text="ACCESO",bg="gray77",width=28,height=2,command=self.init_task).place(x=363,y=280)#295
        Button(self.root,text="BORRAR",bg="gray77",width=28,height=2,command=self.remove_link).place(x=363,y=350)#365
        Button(self.root,text="BORRAR TODO",bg="gray77",width=28,height=2,command=self.delete_listbox).place(x=363,y=400)#415
        Button(self.root,text="LIMPIAR SELECCION",bg="gray77",width=28,height=2,command=self.clear_selection).place(x=363,y=450)#465
        self.selMod = Button(self.root,text="MODO SELECCION: NORMAL",bg="gray77",width=28,height=2,command=self.selection_mode)
        self.selMod.place(x=363,y=500)#515
        Button(self.root,text="GUARDAR LISTA",bg="gray77",width=28,height=2,command=self.write_doc).place(x=363,y=570)#575
    ...
    def copy_paste(self):
        messagebox.showinfo("COPIAR URL","Copia el URL que quieras.")
    ...
    def delete_listbox(self):
        if self.linkBox.size() > 0:
            message = messagebox.askquestion("REMOVER",'¿Quieres remover todos los links de la lista?')
            if message == "yes":
                self.link_list = {}
                self.update_json()
                self.linkBox.delete(0,END)
    ...
    def selection_mode(self):
        if self.selMode == 'normal':
            ...
            self.selMod.configure(text="MODO DE SELECCION: MULTIPLE")
            self.selMode = 'multiple'
        else:
            self.linkBox.configure(selectmode='normal')
            self.selMod.configure(text="MODO DE SELECCION: NORMAL")
            self.selMode = 'normal'
        ...
        if self.urlEntry.get() != "":
            if self.urlEntry.get() not in self.link_list.values():
                ...
                if is_url:
                    ...
                    self.window.title("Nombre Link")
                    Label(self.window,text="INGRESA EL NOMBRE DE LINK",width=67).place(x=0,y=45)
                    entry_name = Entry(self.window,width=25,font=('arial',20))
                    entry_name.place(x=44,y=90)
                    Button(self.window,text="Setear nombre",width=10,height=2,bg="gray77",command=lambda:self.set_name(entry_name.get())).place(x=198,y=180)

                else:
                    messagebox.showwarning("URL invalido","Ingresa un URL validoL.")
                    self.my_url.set("")
            else:
                value_name = self.get_key(self.urlEntry.get())
                messagebox.showwarning("YA GUARDADO","El URL ingresado ya esta ingresado como: \'{}\'".format(value_name))
    ...
    def remove_link(self):
        ...
        if any_selected:
            message = messagebox.askquestion("REMOVER LINK",'Eliminar los Links seleccionados?')
            if message == "yes":
    ...
    def open_page(self):
        try:
            ...
            messagebox.showwarning("Problemas de acceso", str(e))
    ...
    def init_task(self):
        ...
        if self.any_selected:
            ...
            messagebox.showwarning("No hay Links seleccionados","Seleccione un Link para ir.")
    ...
    def write_doc(self):
        if self.linkBox.size() > 0:
            doc = filedialog.asksaveasfilename(initialdir="/",
                                               title="Guardar como", initialfile="links guardados",defaultextension=".txt")
            if doc != "":
                new_file = open(doc,"w")
                x = PrettyTable()
                x.field_names = ["Nombre Link", "URL"]
                content = self.create_list_of_lists(self.link_list)
                x.add_rows(content)
                new_file.write(x.get_string())
                new_file.close()
        else:
            messagebox.showwarning("SIN OBJETOS","No hay nada que guardar.")
